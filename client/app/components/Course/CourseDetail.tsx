...
const CourseDetails = ({ data, setOpen: openAuthModal, stripePromise, clientSecret, setRoute }: Props) => {
  ...
  // first we check course id is exist in user model courses section
      ...
      <div className="w-[90%] 800px:w-[90%] m-auto py-5">
        <div className="w-full flex flex-col-reverse 800px:flex-row">
          <div className="w-full 800px:w-[65%] 800px:pr-5">
            ...
            {/* prerequisites array start here  */}
            ...
            <div className="w-full">
              <div className="800px:flex items-center">
                {/* this rating come from utils and eccept rating number only  */}
                <Ratings rating={data?.ratings} />
                <div className="mb-2 800px:mb-[unset]" />
                <h5 className="text-[25px] font-Poppins text-black dark:text-white">
                  {/* this will show total rating of course like 4,5, and reviews length */}
                  {Number.isInteger(data?.ratings) ? data?.ratings.toFixed(1) : data?.ratings.toFixed(2)} Course Rating
                  • {data?.reviews?.length} Reviews
                </h5>
              </div>
              <br />

              {/* here we run a loop on every Elements of review array to show all reviews of single course  */}
