...
const Courses = ({}: Props) => {
  ...
  useEffect(() => {
    ...
    if (category !== 'All') {
      setcourses(data?.allCourse.filter((item: any) => item.categories === category));
    }
    if (search) {
      setcourses(data?.allCourse.filter((item: any) => item.name.toLowerCase().includes(search.toLowerCase())));
    }
